...
export const useItemSectionAside = () => {
  ...
  const { copper, electron, gold, platinum, silver } = character.itens.coins
  const coinsWeight = +copper + +electron + +gold + +platinum + +silver
  const currentWeight =
    listWeight.reduce((acc, current) => acc + current) + coinsWeight
  ...
  return { exploration, journey, encounter, coins, currentWeight }
}
